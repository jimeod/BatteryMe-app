    ...
    <div class="home-header">
      <div class="logo-row">
        <div class="logo-circle">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#3ecfcf" stroke-width="2.5" stroke-linecap="round">
      ...
      <div class="circ-area">
        <div class="circ-wrap">
          <svg viewBox="0 0 110 110" class="circ-svg">
            ...
            <circle cx="55" cy="55" r="45" fill="none" stroke="#3ecfcf"
...
watch(() => store.state.battery, (val) => animateTo(val))
...
.blob-tr {
  ...
  background: #c8f0ee; opacity: 0.5;
  border-radius: 60% 80% 70% 50% / 70% 50% 80% 60%;
  animation: blobFloat 6s ease-in-out infinite;
  pointer-events: none;
}
.dot { position: absolute; border-radius: 50%; background: var(--teal); animation: dotFloat 4s ease-in-out infinite; pointer-events: none; }
.dot-a { width: 8px; height: 8px; top: 70px; left: 18px; opacity: 0.35; }
.dot-b { width: 5px; height: 5px; top: 120px; right: 30px; opacity: 0.25; animation-delay: 1.5s; }
@keyframes blobFloat { 0%,100%{transform:scale(1) rotate(0deg);} 50%{transform:scale(1.06) rotate(5deg);} }
...
.avatar-btn:hover { background: var(--teal-dark); }
...
.bat-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
.bat-label { font-size: 10.5px; font-weight: 600; color: var(--muted); text-transform: uppercase; letter-spacing: 0.5px; }
.bat-pct { font-family: 'DM Serif Display', serif; font-size: 18px; color: var(--teal); }

.circ-area { display: flex; justify-content: center; margin-bottom: 12px; }
.circ-wrap { position: relative; width: 115px; height: 115px; }
.circ-svg { width: 100%; height: 100%; }
.arc { transition: stroke-dashoffset 0.7s cubic-bezier(0.22, 0.68, 0, 1.2); }
.circ-inner {
  position: absolute; inset: 0;
  display: flex; flex-direction: column; align-items: center; justify-content: center;
}
.circ-num { font-family: 'DM Serif Display', serif; font-size: 30px; color: var(--text); line-height: 1; }
.circ-sym { font-size: 14px; color: var(--teal); }
.circ-status { font-size: 9.5px; color: var(--muted); text-align: center; margin-top: 3px; max-width: 74px; line-height: 1.3; }

.dots-row { display: flex; gap: 4px; justify-content: center; }
.sdot { width: 22px; height: 6px; border-radius: 3px; background: #e0f0f0; transition: background 0.4s; }
...
.count-pill {
  ...
  background: rgba(62,207,207,0.12); padding: 3px 10px; border-radius: 20px;
}
.acts-list { display: flex; flex-direction: column; gap: 9px; position: relative; z-index: 2; }
...
.pop-enter-active { animation: popIn 0.5s cubic-bezier(0.22,0.68,0,1.4) both; }
